...
The wedge: omnigent already solves orchestration (sub-agents, policies, worktrees, sandboxes, multi-harness) and exposes a rich SSE event taxonomy; its first-party desktop client (`web/`, formerly `ap-web` — an Electron shell over the server-served React SPA) is *polished and actively developed* but **single-server, single-warm-stream, chat-shaped**: one live session streamed at a time (`switchTo` aborts the prior stream), one server origin per window. Lens is the supervisor surface that shape can't reach — **multi-server, N-warm-streams** (every session's stream kept live off-thread → zero switch latency, cards always showing live state), plus diff review, Bridge (actionable queue + agent-to-agent relay + todos), fleet dashboard, mid-flight steering, a Canvas for agent-presented visuals, and a long-standing Concierge agent — all native, at fleet scale. (Fresh re-read of `web/` on 2026-07-06, omnigent `62b4254a`, confirms the mismatch is architectural, not a maturity gap.)
...
| C. Terminal reconnect UX (retained emulator vs blank-on-reconnect) | **Resolved** — one bounded Ghostty engine survives brief transport reconnects; deliberate Sleep releases the engine/full scrollback and keeps only a final viewport                                                                                                                                                                         | workspace & terminals                |
...
| G. Multi-window posture                                      | **Resolved** — both (gpui multi-window native); **detach = move, a conversation is shown in ≤1 window** (raise-don't-clone)                                                                                                                                                                                 | application shell                    |
