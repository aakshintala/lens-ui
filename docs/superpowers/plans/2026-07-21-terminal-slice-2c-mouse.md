...
- **Read-only suppresses reporting, allows select/copy.** The engine reports iff `write_allowed && live_tracking != None && !shift && !mouse_local && policy == Auto`. Under read-only a gesture over active tracking becomes **local selection** (Left) or no-op (Right/Middle). Copy/SelectAll are never gated.
