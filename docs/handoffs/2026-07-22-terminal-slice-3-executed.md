...
**Verification note:** headless-verified — `clippy` clean, `inspect` lib tests 16/16 (covers the ordering fix), round-1 windowed pass of the I6/I7 harness. The round-2-strengthened `stream_perf_realwindow` windowed run frame-starves under background-window throttling in a headless subprocess (gpui pumps frames only for a foreground window; System Events can't front the non-bundled test binary); its strict additions live in the exit block and only trip on anomalies round-1 disproved. Final windowed confirmation runs via the macOS `xtask gate` (foreground) or a manual foreground run.
