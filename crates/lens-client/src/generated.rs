// @generated by `cargo run -p xtask -- codegen` from vendor/omnigent-0.4.0/openapi.json — DO NOT EDIT BY HAND.
#![allow(clippy::all)]
...
pub struct LaunchRunnerRequest {
    ///Optional git worktree options. When set, the server creates a worktree for a new branch off `workspace` on the host and binds the runner to it (the fork-resume path; mirrors `POST /v1/sessions`). `None` binds `workspace` directly. `host_id` is always present (it is in the path), so no host requirement check is needed here.
    #[serde(default, skip_serializing_if = "::std::option::Option::is_none")]
...
///    "tier": {
///      "title": "Tier",
///      "description": "The difficulty tier the router assigned, one of `\"cheap\"` / `\"medium\"` / `\"expensive\"`, e.g. `\"expensive\"`.",
///      "type": "string",
///      "enum": [
///        "cheap",
///        "medium",
...
pub struct RoutingDecisionData {
    ...
    ///The difficulty tier the router assigned, one of `"cheap"` / `"medium"` / `"expensive"`, e.g. `"expensive"`.
    pub tier: Tier,
}
...
pub struct SessionForkRequest {
    ...
    #[serde(default, skip_serializing_if = "::std::option::Option::is_none")]
    pub agent_id: ::std::option::Option<::std::string::String>,
    ///Model id to launch the fork on, e.g. `"databricks-gpt-5-4-mini"` — the "restart with model" path. Overrides the model the fork would otherwise inherit from the source; the value is validated and family-checked against the fork's harness. When `None` (default), the fork keeps the source's model (within the same provider family).
    #[serde(default, skip_serializing_if = "::std::option::Option::is_none")]
...
pub struct SessionGitOptions {
    ///Optional base ref to branch from, e.g. `"main"` or `"origin/main"`. `None` branches from the source repository's current `HEAD`.
    #[serde(default, skip_serializing_if = "::std::option::Option::is_none")]
    pub base_branch: ::std::option::Option<::std::string::String>,
    ///Name of the new branch to create and check out in the worktree, e.g. `"feature/login"`. Validated against git ref-format rules; invalid names fail with `invalid_input`.
...
impl ::std::fmt::Display for Tier {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        match *self {
            Self::Cheap => f.write_str("cheap"),
            Self::Medium => f.write_str("medium"),
            Self::Expensive => f.write_str("expensive"),
        }
    }
}
