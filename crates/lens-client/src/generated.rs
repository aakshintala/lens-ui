// @generated by `cargo run -p xtask -- codegen` from vendor/omnigent-0.3.0/openapi.json — DO NOT EDIT BY HAND.
#![allow(clippy::all)]
...
///    "model_override": {
///      "title": "Model Override",
///      "description": "Per-session LLM model override, e.g. `\"claude-opus-4-7\"`. `None` means no override is active (the agent's `llm_model` applies). Set via `PATCH /v1/sessions/{id}` or the REPL's `/model` command; both write the same column so the ap-web UI and the TUI stay in sync.",
///      "anyOf": [
///        {
///          "type": "string"
///        },
...
pub struct SessionResponse {
    ...
    pub model_options:
        ::std::vec::Vec<::serde_json::Map<::std::string::String, ::serde_json::Value>>,
    ///Per-session LLM model override, e.g. `"claude-opus-4-7"`. `None` means no override is active (the agent's `llm_model` applies). Set via `PATCH /v1/sessions/{id}` or the REPL's `/model` command; both write the same column so the ap-web UI and the TUI stay in sync.
    #[serde(default, skip_serializing_if = "::std::option::Option::is_none")]
    pub model_override: ::std::option::Option<::std::string::String>,
    ...
    #[serde(default, skip_serializing_if = "::std::vec::Vec::is_empty")]
    pub skills: ::std::vec::Vec<SkillSummary>,
    ///Session lifecycle status. One of `"idle"` (no loop running), `"running"` (loop executing), or `"failed"` (terminal failure).
    pub status: Status,
...
pub struct UpdateSessionRequest {
    ...
    #[serde(default, skip_serializing_if = "::std::option::Option::is_none")]
    pub runner_id: ::std::option::Option<::std::string::String>,
    ///When `True`, persist metadata changes but skip the runner-side side effects — specifically the native `/effort` / `/model` / Codex collaboration-mode forwards into the live runtime. Used by automatic bind-time handoffs (ap-web's sticky-pref apply on session switch, the REPL's pre-create `/model` snapshot) where injecting a visible slash command into a freshly-spawned pane would render as an unexpected "Command model X" item before the user has sent anything. Default `False` preserves the user-driven picker / `/model` behaviour where the live forward IS the desired feedback.
    #[serde(default)]
    pub silent: bool,
