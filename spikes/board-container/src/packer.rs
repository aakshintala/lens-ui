//! Pure port of `pack()` / `foot()` from the pixel SSOT
//! (`docs/design/renders/board-home.html`). Grid-snap first-fit with
//! hole-backfill, ordinal order. Yields per-tile grid coords + footprint;
//! pixel geometry is derived by the helpers below. No gpui here — this is the
//! algorithm the B-2 plan will lift into `lens-core` (pure, testable).

// Geometry constants — mockup values from the SSOT (§2.1). Tune on device.
...
mod tests {
    ...
    #[test]
    fn hole_backfill() {
        // A 2×2 group then two 1×1 cards in 3 cols: the cards backfill the
        // holes beside the group (col 2, rows 0 and 1) — ordinal order kept.
