...
`…/mcp`. Their request/response schemas (`SessionEventInput`, `ElicitationResult`)
consequently drop out of `components/schemas` even though the wire shape is
unchanged — `lens-client` hand-authors those types where it wraps the hidden
