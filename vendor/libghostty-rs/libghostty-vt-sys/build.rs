...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=ZIG"); // LENS LOCAL PATCH (see below)
...
fn build_vendored(link_mode: LinkMode) {
    ...
    // LENS LOCAL PATCH: macOS 26 requires the patched Homebrew/Nix zig@0.15
    // (keg-only, not on PATH — see vendor/libghostty-rs/README.md). Honor a
    // `ZIG` override (set in .cargo/config.toml) so plain `cargo build/test`
    // finds it without a per-shell PATH edit; default to `zig` on PATH.
    // Re-apply this one-liner on every pin bump.
